...
class PageThree(Frame):
    def __init__(self, parent, controller):
        ...
        for a in range(61):
            if(a!=0):
                val.append(a)
        TA_A = Combobox(self, values=(val), width=3,justify="center",state='readonly')
        TA_A.set("3")
        TA_A.place(x=258, y=87)
        TB_A = Combobox(self, values=(val), width=3,justify="center",state='readonly')
        TB_A.set("10")
        TB_A.place(x=345, y=87)
        TC_A = Combobox(self, values=(val), width=3,justify="center",state='readonly')
        TC_A.set("15")
        TC_A.place(x=425, y=87)
        TA_B = Combobox(self, values=(val), width=3,justify="center",state='readonly')
        TA_B.set("4")
        TA_B.place(x=258,y=145)
        TB_B = Combobox(self, values=(val), width=3,justify="center",state='readonly')
        TB_B.set("12")
        TB_B.place(x=345, y=145)
        TC_B = Combobox(self, values=(val), width=3,justify="center",state='readonly')
        TC_B.set("18")
        TC_B.place(x=425, y=145)
        TA_C = Combobox(self, values=(val), width=3,justify="center",state='readonly')
        TA_C.set("5")
        TA_C.place(x=258, y=220)
        TB_C = Combobox(self, values=(val), width=3,justify="center",state='readonly')
        TB_C.set("15")
        TB_C.place(x=345, y=220)
        TC_C = Combobox(self, values=(val), width=3,justify="center",state='readonly')
        TC_C.set("20")
